...
import { updateWealthPosition } from "@/app/actions/wealth";
import {
  WealthPositionForm,
...
export function EditWealthPositionDialog({
  position,
  accounts,
  open,
  onOpenChange,
}: {
  position: WealthPositionRow;
  accounts: Account[];
  open: boolean;
  ...
  const formId = `edit-wealth-position-${position.id}`;

  function handleSubmit(values: WealthPositionFormValues) {
    startTransition(async () => {
      const result = await updateWealthPosition(position.id, values);
      if (result.error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-[26px] border-[#F1EFF7] sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-[20px] font-extrabold">Edit position</DialogTitle>
          <DialogDescription className="text-sm font-semibold text-[#6E6B82]">
            Update the balance you track for {position.name}.
          </DialogDescription>
        </DialogHeader>
